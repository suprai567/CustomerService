...
export class RequestService {
  ...
  createRequest(customerName: string, summary: string, status: string, assignedGroup: string, serviceType: ServiceType, createdDate: Date) {
    this.request = new Request(1, customerName, summary, status, assignedGroup, serviceType, createdDate);
  ...
  updateUserRequest(ticketId: number, customerName: string, summary: string, status: string, assignedGroup: string, serviceType: ServiceType, createdDate: Date) {
    ...
    //const ticketId = this.user.id;
    return this.client.put(API_URL + "/ticket/" + this.request.ticketId + "/tickets/updateUserRequest/", this.request);
  }
  deleteRequest() {
    const ticketId = this.user.id;
    return this.client.delete(API_URL + "/ticket/" + ticketId + "/ticket/deleteRequest");
